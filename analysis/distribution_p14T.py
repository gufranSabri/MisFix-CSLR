...
def beam_decode(logits, word_to_id, lengths, blank_index=0, beam_width=10):
...
if __name__ == "__main__":
    ...
    gloss_dict_path = "../assets/phoenix2014-T/gloss_dict.npy"
    ...
    id2gloss = {v: k for k, v in gloss_dict.items()}

    # ------------------ Load test info ------------------
    info_path = f"../assets/phoenix2014-T/{mode}_info_ml.npy"
    info = np.load(info_path, allow_pickle=True).item()
    del info["prefix"]  # remove prefix if exists


    logits_path = f"/Users/gufran/Developer/data/sign/phoenix2014-T/sf_logits/{mode}"
    logits_files = os.listdir(logits_path)
    logits_files_ids = [l.replace(".npy", "") for l in logits_files]

    labels = [info[k]['gloss'] for k in info.keys() if info[k]["fileid"] in logits_files_ids]
    fileids = [info[k]["fileid"] for k in info.keys() if info[k]["fileid"] in logits_files_ids]

    logits_files = [f+".npy" for f in fileids]
    ...
    for i in tqdm(range(0, B, batch_size)):
        ...
        decoded = list(torch.argmax(torch.tensor(logits_batch), dim=-1).numpy())

        label = labels[i].split(" ")
        label = [gloss_dict[gl] for gl in label]
        ...
        decoded = [decoded[j].item() for j in range(len(decoded)) if j in indices]
        ...
        for j in range(len(decoded)):
            if decoded[j] != label[j]:
                logit = logits_batch[j]
                top_indices = np.argsort(logit)[-k:][::-1]  #
                top_glosses = [id2gloss[idx] for idx in top_indices]
                
                print(f"Position {j}: GT={id2gloss[label[j]]}, Pred={id2gloss[decoded[j]]}, Top-10={top_glosses}")
                if id2gloss[label[j]] in top_glosses:
                    # topked.append(j)
                    print(True)

                # break

    # print(f"Total positions where GT is in Top-{k} predictions: {len(topked)} out of {B}")